...
def dp_1norm(data, domain, index_list, marginal, noise, to_cpu=False):
    ...
    shift_index_list = [domain.attr_list.index(attr) for attr in index_list]
    if carest is True:
...
def dp_TVD(TVD_map, data, domain, index_list, noise):
    all_attr_list = domain.attr_list
    shift_index_list = [all_attr_list.index(attr) for attr in index_list]

    # print(index_list, shift_index_list)
    ...
    if index_list not in TVD_map:
        ...
        temp_index_list = [all_attr_list.index(attr) for attr in temp_domain.attr_list]
        if carest is True:
...
def dp_entropy(entropy_map, data, domain, index_list, noise):
    # print(index_list)
    shift_index_list = tuple(sorted([domain.attr_list.index(attr) for attr in index_list]))
    # print(index_list, shift_index_list)
    ...
    if index_list not in entropy_map:
        ...
        if len(index_list) <= 12 and size < 5e6:
            ...
            if carest is True:
                ...
                value, counts = np.unique(data[:, shift_index_list], return_counts=True, axis=0)
...
def dp_marginal_list(data, domain, marginal_list, noise, return_factor=True, valid=False, noise_type='normal'):
    ...
    for marginal in marginal_list:
        ...
        shift_marginal = [domain.attr_list.index(attr) for attr in temp_domain.attr_list]
        if carest is True:
            histogram = histogramdd(marginal, temp_domain.edge())
            # print(marginal, temp_domain.edge(), histogram)
        else:
            histogram, _ = np.histogramdd(data[:, shift_marginal], bins=temp_domain.edge())
...
def privacy_budget(epsilon):
    f = {
        ...
        0.250: 0.005665556091116741,
        0.260: 0.006088521296624095,
        0.270: 0.006525340316584334,
        0.280: 0.006975927390158176,
        0.290: 0.0074401905445847655,
        0.300: 0.007918054901529107,
        0.310: 0.008409438688540834,
        0.320: 0.008914261125028133,
        0.330: 0.00943246001726948,
        0.360: 0.011066538281738758,
        0.370: 0.01163750672130846,
        0.380: 0.012221498184371737,
        0.400: 0.013428307138383389,
        0.480: 0.01876466441899538,
        0.490: 0.01948807606415358,
        0.530: 0.02250478163186926,
        0.540: 0.023289482501568273,
        0.550: 0.024086300333146946,
        ...
        1.080: 0.08265503092319706,
        1.090: 0.0840522112059989,
        1.110: 0.08545968962425834,
        ...
        2.100: 0.27450272619462346,
        2.110: 0.27684392924347767,
        2.120: 0.2791934984194813,
        2.130: 0.28155141911542164,
        2.140: 0.2839176767689058,
        2.150: 0.28629225679833326,
        2.160: 0.28867514472571204,
        2.170: 0.2910663261766603,
        2.200: 0.2982894885208225,
