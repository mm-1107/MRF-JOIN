...
def k_way_marginal(data_name, dp_data_list, k, marginal_num):
    ...
    # print(f"domain.keys()={domain.keys()}, marginal_num={marginal_num}")
    domain = Domain(domain, list(domain.keys()))
    ...
    for marginal in marginal_list:
        shift_marginal = [domain.attr_list[idx] for idx in marginal]
        temp_domain = domain.project(shift_marginal)
...
def cross_validation_data(data_name):
    ...
    _, headings = utils.tools.read_csv('./exp_data/' + data_name + '_train.csv')
...
def svm_exp(data_name_list, method_list, exp_name, \
    epsilon_list, repeat=10, marginal_num=300, classifier_num=10, generate=True, party="A"):
    result = {}
                ...
                for i in range(repeat):
                    ...
                    # for j in range(4, 5):
                        ...
                        for method in method_list:
                            ...
                            run_syn(data_name, temp_exp_name, epsilon, task='SVM', party=party)
    else:
        print_lock = mp.Lock()
        result_lock = mp.Lock()
...
def marginal_exp(data_list, method_list, exp_name, \
    epsilon_list, repeat=10, marginal_num=300, classifier_num=10, party="A"):
        ...
        for data_name in data_list:
            ...
            for i in range(repeat):
                ...
                for method in method_list:
                    ...
                    if method == 'PrivMRF':
                        dp_data_list.append(run_syn(data_name, temp_exp_name, epsilon, task='TVD', party=party))
