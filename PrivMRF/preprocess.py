...
def preprocess(data, party="", num_party=1, num_attr=0, common_attr=[], headings=[], exhead=[]):
    ...
    print(f"Party {party}, headings={headings}")
    filter = [True if eval(x) in headings else False for x in all_headings]
    data_list = data_list[:, filter]
    # print("excluded", data_list[:2])
    ...
    json_domain = {attr: {'type': 'discrete'} for attr in headings}
    max_level = []
    # for attr in range(attr_num):
    for attr in headings:
        max_level.append(max(attr_list[attr].level_to_size.keys()))

    # value: str to int
    value_id_to_vn = [0]*len(all_headings)
    value_to_value_id = [0]*len(all_headings)


    for idx, attr in enumerate(headings):
    # for attr in headings:
        # attr = int(attr)
        value_set = set()
        for line in data_list:
            if line[idx] not in value_set:
                value_set.add(line[idx])
    ...
    domain = Domain(json_domain, headings)

    # print("cal_privacy_budget epsilon 80", tools.cal_privacy_budget(epsilon=80))
    ...
    new_data_list = []
    for line in data_list:
        new_line = [value_to_value_id[attr][line[idx]] for idx, attr in enumerate(headings)]
    ...
    return data, domain, attr_list, headings
...
def read_preprocessed_data(data, task='TVD'):
    ...
    headings = [eval(idx) for idx in headings]
    print(json_domain, headings)
    domain = Domain(json_domain, headings)
