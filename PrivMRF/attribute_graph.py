...
class Status:
    ...
    def get_neighbor_status(self, attr1, attr2, weight, attr_list):
        neighbor_status = Status(self.graph, self.adj)
        idx1 = attr_list.index(attr1)
        idx2 = attr_list.index(attr2)
        if neighbor_status.adj[idx1, idx2] > 0:
            neighbor_status.adj[idx1, idx2] = 0
            neighbor_status.adj[idx2, idx1] = 0
            neighbor_status.graph.remove_edge(attr1, attr2)
            return neighbor_status
        elif neighbor_status.adj[idx1, idx2] == 0:
            neighbor_status.adj[idx1, idx2] = weight
            neighbor_status.adj[idx2, idx1] = weight
...
class AttributeGraph:
    def __init__(self, data, domain, attr_list, config, data_name):
        ...
        if self.config['beta2'] > 0:
            ...
            if self.config['epsilon'] == float('inf'):
                self.max_measure_dom_2way = 8
                # TODO
                self.max_measure_dom_high_way = 8
            else:
    ...
    def construct_model(self):
        ...
        print(f"self.config['enable_attribute_hierarchy']={self.config['enable_attribute_hierarchy']}")
    ...
    def local_search(self):
        ...
        # print(f"--> start_G.edges={start_G.edges()}")
        # print("self.domain.attr_list=", self.domain.attr_list)
        start_G.add_nodes_from(self.domain.attr_list)
        print(f"--> start_G.nodes={start_G.nodes(data=False)}")
        ...
        temp1 = 0
        # temp2 = 0
        for idx1 in range(self.attr_num):
            attr1 = self.domain.attr_list[idx1]
            for idx2 in range(idx1+1, self.attr_num):
                attr2 = self.domain.attr_list[idx2]
        ...
        for i in range(search_iter_num):
            ...
            best_status = None
            edge_list = []
            # for idx_1, attr1 in enumerate(self.domain.attr_list):
                # for idx_2, attr2 in enumerate(self.domain.attr_list[idx_1+1:]):
            for idx1 in range(self.attr_num):
                attr1 = self.domain.attr_list[idx1]
                for idx2 in range(idx1+1, self.attr_num):
                    # print(attr1, attr2)
                    attr2 = self.domain.attr_list[idx2]
                    if current_status.adj[idx1][idx2] == 0:
                        edge_list.append((attr1, attr2))
            random.shuffle(edge_list)
            # print(f"--> current_status.nodes={current_status.graph.nodes(data=False)}")
            # print(f"--> current_status.edges={current_status.graph.edges(data=True)}")
            for attr1, attr2 in edge_list:
                status = current_status.get_neighbor_status(attr1, attr2, 1, self.domain.attr_list)
    ...
    def maximal_parents(self, parents_set, dom):
        ...
        # debug
        attr_index = self.domain.attr_list.index(attr)
        if self.config['enable_attribute_hierarchy']:
            level = max(self.max_level[attr_index]-2, 0)
        else:
            level = max(self.max_level[attr_index], 0)

        current_attr_size = self.attr_list[attr_index].level_to_size[level]
    ...
    def attr_parents_score(self, attr, parents):
        ...
        for i in range(len(parents)):
            for j in range(i+1, len(parents)):
                if self.config['score'] == 'pairwsie_TVD':
                    denominator += tools.dp_TVD(self.TVD_map, self.data, self.domain, [parents[i], parents[j]], self.TVD_noise)[1]
                elif self.config['score'] == 'pairwsie_MI':
                    numerator += tools.dp_mutual_info(self.MI_map, self.entropy_map, self.data, self.domain, [parents[i], parents[j]], self.MI_noise)[1]
    ...
    def score(self, graph, entropy_map):
        ...
        for clique in clique_list:
            # attr_clique = [self.domain.attr_list[idx] for idx in clique]
